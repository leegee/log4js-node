...
function patternLayout (pattern, tokens, timezoneOffset) {
  ...
  function className(loggingEvent, specifier) {
    loggingEvent._caller = loggingEvent._caller || _getCaller(loggingEvent.callStack);
    var str = loggingEvent._caller
  ...
  // As in Log4j, this is expensive
  function _getCaller (callStack){
    callStack = callStack.stack.split("\n");
    // Call is known to originate internally:
    var origin = callStack[1].replace(
      /^.+?\sat\s.*?\s\((.+?).lib.logger\.js:\d+:\d+\)$/,
      function (all, initialPath){ return arguments[1] }
    );
    // In case of problems:
    var rv = '<UNKNOWN>';
    // Find the first line not in the same directory as this package:
    var depth=2;
    for (depth; depth<callStack.length; depth++){
      if ( ! callStack[depth].match( origin ) ) {
        rv = callStack[depth-1];
        break;
      }
    }
    return rv;
